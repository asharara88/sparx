...
export async function verifyClaim(claim: string): Promise<FactCheckClaim> {
  const search = await webResearch(claim, 4).catch(() => ({ query: claim, results: [], live: false }));
  ...
  const res = await llm.complete({
    ...
    prompt: `Claim: ${claim}\n\nEvidence:\n${evidence}\n\nReturn JSON {"verdict":"supported"|"unsupported"|"uncertain","note":"one sentence citing the evidence number"}.`,
    mock: JSON.stringify({ verdict: 'uncertain', note: 'mock verdict — no live LLM' }),
  });
  const v = res.data!;
  return { claim, verdict: v.verdict as ClaimVerdict, source: results[0]!.url, note: v.note };
}
